...
public:
	...
	double line_search(double *delta_w, double wTd, double *f, double eta, double alpha);
...
double l2r_lr_fun::fun(double *w)
{
	...
	for(i=0;i<l;i++)
	{
		yz[i] = y[i]*z[i];
		if (yz[i] >= 0)
			f += C[i]*log(1 + exp(-yz[i]));
		else
			f += C[i]*(-yz[i]+log(1 + exp(yz[i])));
	}
	f += reg/2;
...
double l2r_lr_fun::line_search(double *delta_w, double wTd, double *f, double eta, double alpha)
{
	int i;
	int l = prob->l;
	double *y=prob->y;
	...
	double dTd = 0;
	for (i=0;i<w_size;i++)
		dTd += delta_w[i] * delta_w[i];

	int line_search_times = 0;
	while (true)
	{
		if (line_search_times >= 1000)
		{
			f[0] = current_f;
			return 0;
		}
		...
		for(i=0;i<l;i++)
		{
			double inner_product = z[i] * alpha + yz[i];
			if (inner_product >= 0)
				loss += C[i]*log(1 + exp(-inner_product));
			else
				loss += C[i]*(-inner_product+log(1 + exp(inner_product)));
		}
		f[0] = loss + (alpha * alpha * dTd + reg) / 2 + alpha * wTd;
		if (current_f - f[0] > alpha * alpha * eta * dTd)
		{
			for (i=0;i<l;i++)
			{
				yz[i] += alpha * z[i];
	...
	reg = reg + alpha*alpha * dTd + 2* alpha * wTd;
	current_f = f[0];
	return alpha;
}
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		case LBFGS_SOLVER:
		{
			LBFGS lbfgs_obj(fun_obj, primal_solver_tol, param->m, param->eta);
		...
		case NEWTON_SOLVER:
		{
			NEWTON newton_obj(fun_obj, primal_solver_tol, param->eta);
		...
		case AFG_SOLVER:
		{
			AFG afg_obj(fun_obj, primal_solver_tol, 1.0 / (param->C * prob->l), 1.0);
			afg_obj.set_print_string(liblinear_print_string);
			afg_obj.afg(w);
			break;
		}
		default:
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != LBFGS_SOLVER && param->solver_type != NEWTON_SOLVER && param->solver_type != AFG_SOLVER)
