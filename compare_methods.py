...
f1 = open('figures/methods_time.tex','w')
f2 = open('figures/methods_pass.tex','w')
...
for c in loss_param:
	...
	for d in data:
		...
		for method in methodlist:
			dp = 'log/%s.LRC%s.%s'%(d,c, method)
			try:
				tmp_data = open(dp,'r').readlines()
			except:
				traindata = 'data/'+ d
				model = 'model/%s.C%s.%s.model'%(d,c,method)
				cmd = ''
				m = 1000
				if (method == 'lbfgs30'):
					m = 30
				if (method != 'commdir'):
					cmd = 'other_solvers/train -e 1e-6 -s %s -c %s -m %s %s %s >> %s'%(solver[method], c, m, traindata, model, dp)
		...
		print>>f1, '\\plotfunc{%s.LRC%s}{\\%s}'%(d,c,dname[d])
		print>>f2, '\\plotpass{%s.LRC%s}{\\%s}'%(d,c,dname[d])
	print >>f1, '\\caption{Training time of logistic regression with $C=%s$.}'%c
	print >>f2, '\\caption{Number of data passes of logistic regression with $C=%s$.}'%c
	print >> f1, '\\end{figure}'
	print >> f2, '\\end{figure}'
...
import gentable
chdir('figures/')
cmd = 'pdflatex methods_time.tex'
system(cmd)
cmd = 'pdflatex methods_pass.tex'
system(cmd)
cmd = 'make -f methods_time.makefile'
system(cmd)
cmd = 'make -f methods_pass.makefile'
system(cmd)
cmd = 'pdflatex methods_time.tex'
system(cmd)
cmd = 'pdflatex methods_pass.tex'
